...
export class TabletopActionService {
  ...
  createLightReactiveMask(position: PointerCoordinate): GameTableMask {
    const tableMask = this.createGameTableMask(position);
    if (!tableMask) return;
    tableMask.name = '照明反応マスク';
    tableMask.maskType = 'lightReactive';
    tableMask.color = '#1b1b22';
    tableMask.bgcolor = '#050505';
    tableMask.update();
    return tableMask;
  }

  createLightAreaMask(position: PointerCoordinate): GameTableMask {
    const tableMask = this.createGameTableMask(position);
    if (!tableMask) return;
    tableMask.name = '照明エリア';
    tableMask.maskType = 'lightArea';
    tableMask.lightShape = 'rect';
    tableMask.color = '#ffe28a';
    tableMask.bgcolor = '#ffe28a';
    tableMask.update();
    return tableMask;
  }
...
//    fileContext.tag = 'テスト01';
  ...
  private getCreateLightMaskMenu(position: PointerCoordinate): ContextMenuAction {
    return {
      name: '照明マスクを作成', action: null, subActions: [
        {
          name: '照明反応マスクを作成', action: () => {
            this.createLightReactiveMask(position);
            SoundEffect.play(PresetSound.cardPut);
          }
        },
        {
          name: '照明エリアを作成', action: () => {
            this.createLightAreaMask(position);
            SoundEffect.play(PresetSound.cardPut);
          }
        }
      ]
